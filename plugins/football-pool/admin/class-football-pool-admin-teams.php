...
class Football_Pool_Admin_Teams extends Football_Pool_Admin {
	...
	private static function edit( $id ) {
		...
		$cols = array(
					...
					array( 'image', __( 'photo', 'football-pool' ), 'photo', $values['photo'], sprintf( __( 'Image path must be a full URL to the image. Or a path relative to the football pool upload directory (%s)', 'football-pool' ), trailingslashit( FOOTBALLPOOL_UPLOAD_URL . 'teams' ) ) ),
					array( 'image', __( 'flag', 'football-pool' ), 'flag', $values['flag'], sprintf( __( 'Image path must be a full URL to the image. Or a path relative to the football pool upload directory (%s)', 'football-pool' ), trailingslashit( FOOTBALLPOOL_UPLOAD_URL . 'flags' ) ) ),
					array( 'text', __( 'link', 'football-pool' ), 'link', $values['link'], __( 'A link to a website with information about the team. Used on the team page in de plugin.', 'football-pool' ) ),
					array( 'multiline', __( 'comments', 'football-pool' ), 'comments', $values['comments'], __( 'An optional text with extra information about the team that is displayed on the team\'s page.', 'football-pool' ) ),
					array( 'dropdown', __( 'group', 'football-pool' ), 'group_id', $values['group_id'], $groups, '' ),
					array( 'integer', __( 'group order', 'football-pool' ), 'group_order', $values['group_order'], __( 'If teams are placed in a group and the default ordering does not work (when teams have the same points) you can fix the ordering with this number.', 'football-pool' ) ),
					array( 'checkbox', __( 'team is not a temporary team name', 'football-pool' ), 'is_real', $values['is_real'], __( 'Uncheck this box if the team is not a real team, e.g. "Winner match 30".', 'football-pool' ) ),
					array( 'checkbox', __( 'team is active', 'football-pool' ), 'is_active', $values['is_active'], '' ),
	...
	private static function view() {
		...
		$cols = array(
					array( 'text', __( 'team', 'football-pool' ), 'team', '' ),
					array( 'boolean', __( 'active', 'football-pool' ), 'is_active', '' ),
					array( 'boolean', __( 'real team', 'football-pool' ), 'is_real', '' ),
				);
		...
		$bulkactions[] = array( 'activate', __( 'Activate team(s)', 'football-pool' ), __( 'You are about to activate one or more teams.', 'football-pool' ) . ' ' . __( 'Are you sure? `OK` to continue, `Cancel` to stop.', 'football-pool' ) );
		$bulkactions[] = array( 'deactivate', __( 'Deactivate team(s)', 'football-pool' ), __( 'You are about to deactivate one or more teams.', 'football-pool' ) . ' ' . __( 'Are you sure? `OK` to continue, `Cancel` to stop.', 'football-pool' ) );
		$bulkactions[] = array( 'delete', __( 'Delete' ), __( 'You are about to delete one or more teams.', 'football-pool' ) . ' ' . __( 'Are you sure? `OK` to delete, `Cancel` to stop.', 'football-pool' ) );
