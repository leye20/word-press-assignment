...
class Football_Pool_Admin_Score_Calculation extends Football_Pool_Admin {
	 ...
	 * @param array $args
	 *
	 * @return array|void Returns the $params array in cli mode, echoes the $params as a JSON response in AJAX mode or echoes a piece of javascript to refresh the page with the params in the querystring in NOAJAX mode.
	 * @throws Exception
	 */
	public static function process( $is_cli = false, $args = null ) {
		...
		$is_cli = ( $is_cli === true );
		$simple_calculation_method = Football_Pool_Utils::get_fp_option( 'simple_calculation_method', 0, 'int' );
		...
		switch ( $calculation_step ) {
			...
			case 'match_scores':
				...
				if ( $pool->has_matches && count( $match_ids ) > 0 ) {
					...
					while ( $i++ < FOOTBALLPOOL_CALC_STEPSIZE_MATCH ) {
						...
						$user_home = $user_away = $home_score = $away_score = null;
						$has_joker = 0;
						...
						foreach ( $ranking_ids as $ranking_id ) {
							if ( $simple_calculation_method ) {
								...
								if ( count( $matches ) > 0 ) {
									...
									$total_score = $match_id = $match_score = $full = $toto = $goal_bonus = $goal_diff_bonus = 0;
									foreach ( $rows as $row ) {
									...
									$sql = $wpdb->prepare( "INSERT INTO {$prefix}{$new_history_table}
															...
															VALUES ( %d, 0, %d, NOW(), 0, %d, 0, %d, %d, %d, %d, %d, 0 )"
															, $ranking_id
									...
									if ( $match_score === null ) {
										$sql = "INSERT INTO {$prefix}{$new_history_table} 
													( ranking_id, score_order, type, score_date, source_id, user_id, 
														score, full, toto, goal_bonus, goal_diff_bonus, total_score, ranking )
												VALUES
			...
			case 'compute_ranking':
				...
				while ( $i++ < FOOTBALLPOOL_CALC_STEPSIZE_RANKING ) {
					$user_id = $user_ids[$user];
					$ranking_id = $ranking_ids[$ranking];
					$scores_for_ranking = $ranking_order = array();
					$score_for_ranking = 0;
	...
	private static function get_match_results() {
		if ( ! isset( $_SESSION['fp_calc_match_results'] ) ) {
			global $wpdb;
			$prefix = FOOTBALLPOOL_DB_PREFIX;
			$match_results = array();
			...
			$_SESSION['fp_calc_match_results'] = $match_results;
		}
		
		return $_SESSION['fp_calc_match_results'];
	}

	/**
	 * @param int $ranking_id
	 * @param string $date A date in the format "Y-m-d H:i"
	 *
	...
	/**
	 * $val is automatically passed by array_walk function but not used
	 *
	 * @param $val
	 * @param $key
	 */
