...
//									 'shortcodes' => implode( '|', array_map( array( 'self', 'get_shortcode_codes' ), $shortcodes ) ),
	 ...
	 * @param $description
	 * @param $type
	 * @deprecated 2.7.1 Old image input is no longer supported by WordPress. Will be removed in a future version.
	 */
	private static function image_input_old( $label, $key, $value, $description, $type ) {
		trigger_error( 'Use of old image input is deprecated. Upgrade your WordPress install.', E_USER_DEPRECATED);
	...
	public static function admin_header( $title, $subtitle = '', $addnew = '', $extra = '' ) {
		...
		printf( '<h2>%s%s%s</h2>', $title, $addnew, $subtitle );
	 ...
	 */
	protected static function search_box( $search ) {
	...
	protected static function list_table( $cols, $rows, $bulkactions = null, $rowactions = null
		...
		if ( $bulkactions === null ) $bulkactions = array();
		if ( $rowactions === null ) $rowactions = array();
