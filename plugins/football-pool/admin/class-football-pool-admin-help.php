...
class Football_Pool_Admin_Help extends Football_Pool_Admin {
	...
	private static function calc_score( $pool, $home, $away, $user_home, $user_away, $joker_multiplier ) {
		$score = $pool->calc_score( $home, $away, $user_home, $user_away, 0, 0, 0 );
		
		return $joker_multiplier * $score['score'];
	}
	
	private static function echo_score( $pool, $home, $away, $user_home, $user_away, $joker_multiplier, $calculation ) {
		printf( '<span title="outcome via calc_score method = %s">%s</span>'
			, self::calc_score( $pool, $home, $away, $user_home, $user_away, $joker_multiplier )
			, $calculation
		);
	}
	...
	public static function admin() {
		...
		<p>
		There are several ways you can customize the plugin: different scores for correct answers, add bonus questions, add your own rankings, etc. See the contents of this help file for details. If you have any questions, you can ask them on the <a target="_blank" href="http://wordpress.org/support/plugin/football-pool">WordPress support forum</a>.
		</p>
		...
		<tr>
			...
			<td>
				toto points.<br />
				total = <?php self::echo_score( $pool, 3, 1, 1, 0, $joker_multiplier, ( $totopoints ) ); ?>
			</td>
		</tr>
		<tr>
			<td>3-1</td>
			...
			<td>
				...
				total = <?php echo $totopoints; ?> + <?php echo $diffpoints; ?> = 
					<?php self::echo_score( $pool, 3, 1, 2, 0, $joker_multiplier, ( $totopoints + $diffpoints ) ); ?>
			</td>
		</tr>
		<tr>
			<td>3-1</td>
			...
			<td>
				...
				total = <?php echo $totopoints; ?> + <?php echo $goalpoints; ?> = 
					<?php self::echo_score( $pool, 3, 1, 3, 0, $joker_multiplier, ( $totopoints + $goalpoints ) ); ?>
			</td>
		</tr>
		<tr>
			<td>2-1</td>
			...
			<td>
				...
				total = <?php echo $fullpoints; ?> + <?php echo $goalpoints; ?> + <?php echo $goalpoints; ?> = 
					<?php self::echo_score( $pool, 2, 1, 2, 1, $joker_multiplier, ( $fullpoints + ( 2 * $goalpoints ) ) ); ?>
			</td>
		</tr>
		<tr>
			<td>2-1</td>
			...
			<td>
				...
				total = <?php self::echo_score( $pool, 2, 1, 1, 1, $joker_multiplier, ( $goalpoints ) ); ?>
			</td>
		</tr>
		<tr>
			<td>2-1</td>
			<td>0-0</td>
			<td><?php self::echo_score( $pool, 2, 1, 0, 0, $joker_multiplier, 'no points' ); ?></td>
		</tr>
		<tr>
			<td>1-1</td>
			<td>1-1</td>
			<td>
				full points plus two times the goal bonus for the correct amount of goals for the home team and the away team.<br />
				total = <?php echo $fullpoints; ?> + <?php echo $goalpoints; ?> + <?php echo $goalpoints; ?> = 
					<?php self::echo_score( $pool, 1, 1, 1, 1, $joker_multiplier, ( $fullpoints + ( 2 * $goalpoints ) ) ); ?>
			</td>
		</tr>
		<tr>
			<td>1-1</td>
			...
			<td>
				toto points.<br />
				total = <?php self::echo_score( $pool, 1, 1, 0, 0, $joker_multiplier, ( $totopoints ) ); ?>
			</td>
		</tr>
		</table>
		...
		But the plugin also has the ability to calculate a ranking of just a subset of the matches and/or bonus questions (e.g. a ranking for the first half of the season and one for the second half). If you want to use this feature, make a new <a href="?page=footballpool-rankings">ranking</a> and attach the required matches and/or questions to this ranking. This is called the ranking definition. The custom rankings can be used with a ranking shortcode, in a ranking widget or on the ranking and charts page.
		</p>
		...
		<p>
		Only matches with a match date in the past (date lies before the start time of the calculation) are used to calculate the ranking. And the same applies to bonus questions: only questions for which the score date lies before the start of the calculation are added to the total score for the user.
		</p>
		<h3 id="ranking-calculation">Ranking calculation</h3>
		<p>By default an admin will be automatically notified for a (re)calculation of the rankings when saving a match or question, or when changing your pool players. If you want to (temporarily) disable this automatic calculation, e.g. when you want to enter multiple matches at once or when you <a href="#ranking-calculation-wp-cli">use WP-CLI</a> to do the calculation, you may disable this feature in the <a href="?page=footballpool-options#option-section-1">plugin options</a> and do a manual recalculation when you've finished editing.
		</p>
		<div class="help important">
			<p><strong>Important:</strong> Calculating a ranking takes time. The more players or rankings you have, the more time it takes to (re)calculate the ranking tables. All rankings are 'cached' in the database. So, once calculated, your players/visitors shouldn't notice a delay when displaying a ranking, but an admin saving a match outcome or adding the scores for a bonus question will have to wait for the ranking calculations to finish.</p>
		</div>
		<div class="help important">
			<p><strong>Tip:</strong> On my local machine I have found that <a href="?page=footballpool-help#ranking-calculation-wp-cli">using WP-CLI</a> to do the calculation is a lot faster than executing it via the admin interface. In some tests up to 10 times faster!</p>
		</div>
		
		<h3 id="ranking-calculation">Default calculation or simple calculation</h3>
		<p>The plugin calculates the ranking and scores for each player at any point in time (that is, for the points in time when there are matches or questions). This 'historic' data is used by the charts and the shortcodes to display a rank or amount of points at a certain time in a tournament.</p>
		<p>And while this is a pretty cool feature, it comes with the downside that these calculations take a long time to complete. Especially when the tournament is in its final stages (a lot of matches to process) and/or when you have a lot of players in the pool. Also, creating custom rankings adds to this calculation time.<br />
		You can boost calculation with a bigger, better server (<a href="https://youtu.be/ts9UONUMlCg" target="_blank">more power</a> <img src="<?php echo includes_url( 'images/smilies/icon_lol.gif' ); ?>" style="max-height: 1.1em" /> ) or investing time in optimizing the DB performance. But adding more resources or doing optimizations will not always help. Or, this not always possible for you.</p>
		
		<p>The plugin also has the option to <a href="admin.php?page=footballpool-options#option-section-1" target="_blank">switch to a simple calculation</a> method. This calculation does not calculate all values in time, but only the latest status. This drastically reduces the time needed for a calculation because it will only calculate and store the latest ranking. But with the calculation method in simple mode it is therefor no longer possible to use charts. The charts will be automatically hidden, regardless of your settings. And it is no longer possible to use a date parameter in the shortcodes for ranking or score (e.g. <span class="code">[fp-user-rank date="2018-06-24 12:00"]</span>). The plugin will simply ignore the date parameter and will just return the latest status.</p>
		<div class="help important">
			<p><strong>Important:</strong> After a switch to another calculation method, you have to do a calculation to change the data in the database to the calculation method!</p>
		</div>
		<div class="help important">
			<p><strong>Tip:</strong> If you decide to switch to another calculation method when your pool already started, I recommend putting your site in maintenance mode first before you start the calculation. Because when switching the calculation the current ranking will cause strange effects. Another option is to truncate the ranking tables before you switch the mode.</p>
		</div>
...
<pre class="code-block"><code class="bash">$ wp help football-pool</code></pre>
		...
		<p>
		If you want to see all the possible options for a sub commands, use this:
		</p>
<pre class="code-block"><code class="bash">$ wp help football-pool calc</code></pre>
		...
		<div class="help important">
			<p>Please note that for points for bonus questions to be added to the total points for a player, an admin also has to fill in the score date field for that question. The score date is used to determine the order in which points are plotted in the charts and to determine if the question should be added in the ranking calculation. If the score date is not set by the admin, then the score date will be automatically set to the current time and date upon a save of the user answers. If a score date is set in the future, then the points will only be added to the total score when a calculation is started after that date.</p>
		</div>
		...
		*/
		...
		<h3>Style</h3>
		<p>The plugin has some basic styling that will hopefully not interfere with your theme. If you want to change the style of the plugin, you can do so by using a seperate CSS file in a child theme, or by adding rules to the "<a href="customize.php" target="_blank">Additional/Extra CSS</a>" field of your theme (if your theme supports this). Creating a child theme is <a href="https://codex.wordpress.org/Child_Themes#How_to_Create_a_Child_Theme" target="_blank">really simple</a> and is the approach I prefer most.<br />
		...
		<p>Simple shortcode that calculates the amount of users in a league (or all users) times a given amount and returns
		this total sum.
		Can be useful if your players add a stake to the pot and you want to show the total amount in a post or in other part
		of the website.</p>
		<p>League parameter can be entered in the following formats:
		<table>
		...
		$code_block = <<<'EOT'
<?php
// a bit useless example that shows the page ID at the top of every page from the plugin,
