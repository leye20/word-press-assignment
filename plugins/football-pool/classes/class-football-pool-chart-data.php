	...
	******************************************/
	public function score_chart_series( $rows ) {
		...
		foreach ( $rows as $user_id => $data_row ) {
			...
			$data[$user_id]['data'] = array(
											array( __( 'full score', 'football-pool' ), (int) $row['scorefull'] ),
											array( __( 'toto score', 'football-pool' ), $toto ),
											array( __( 'no score', 'football-pool' ), $no_score ),
										);
			if ( $goal_bonus_enabled ) {
				$data[$user_id]['data'][] = array( __( 'just the goal bonus', 'football-pool' )
												, (int) $row['goalbonus'] );
			}
			if ( $goal_diff_bonus_enabled ) {
				$data[$user_id]['data'][] = array( __( 'goal difference bonus', 'football-pool' )
												, (int) $row['diffbonus'] );
			}
		}
	...
	private function per_match_line_series( $lines ) {
		...
		if ( count( $lines ) > 0 ) {
			...
			foreach ( $lines as $datarow ) {
				...
				if ( ! array_key_exists( $user_id, $seriesdata ) ) {
					$seriesdata[$user_id] = array(
												'name' => $user, 
												'data' => array() 
											);
				}
				...
				$seriesdata[$user_id]['data'][] = (int) $datarow['value'];
			}
