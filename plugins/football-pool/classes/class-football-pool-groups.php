...
class Football_Pool_Groups {
	...
	private function order_groups( $arr ) {
		foreach ( $arr as $group => $teams ) {
			$sort_function = apply_filters( 'footballpool_group_sort_function', array( $this, 'compare_teams' ) );
			// ugly way to hide the "uasort() array was modified by the user comparison function" warning
			@uasort( $arr[$group], $sort_function );
		}
		return $arr;
	}
			...
			if ( $group_id == '' || $group_id == $group ) {
				...
				foreach ( $rank as $teamranking ) {
					$row_template_params = array(
												...
												'team_id' => $teamranking['team'],
												'css_class' => $class,
												'plays' => $teamranking['plays'],
												'wins' => $teamranking['wins'],
												'draws' => $teamranking['draws'],
												'losses' => $teamranking['losses'],
												'points' => $teamranking['points'],
												'goals_for' => $teamranking['for'],
												'goals_against' => $teamranking['against'],
											);
					...
					if ( $teams->show_team_links ) {
						$row_template_params['team_name'] = sprintf( '<a href="%s">%s</a>'
																		...
																		, Football_Pool_Utils::xssafe( $team_names[$teamranking['team']] )
																	);
					} else {
						$row_template_params['team_name'] = Football_Pool_Utils::xssafe( $team_names[$teamranking['team']] );
					}
					
					// allow for extra fields to be added to the template
					$row_template_params = apply_filters( 'footballpool_group_table_row_template_params'
															, $row_template_params, $teamranking, $layout );
