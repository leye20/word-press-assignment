...
class Football_Pool_Pool {
	...
	public function __construct( $season = 0 ) {
		$this->simple_calculation_method = ( Football_Pool_Utils::get_fp_option( 'simple_calculation_method', 0, 'int' ) === 1 );
	...
	public function get_ranking_from_score_history( 
		...
		if ( $this->has_leagues ) {
			if ( $score_date !== '' ) {
				...
				if ( $league > FOOTBALLPOOL_LEAGUE_ALL ) {
					$sql = $wpdb->prepare( $sql, $ranking_id, $league, $ranking_id );
				} else {
	...
	public function print_bonus_question( $question, $nr, $is_user_page = false ) {
		...
		if ( $this->question_is_editable( $question['question_timestamp'] ) ) {
			if ( ! $is_user_page ) {
				...
				if ( ( $timestamp - current_time( 'timestamp' ) ) <= ( DAY_IN_SECONDS ) ) {
			...
			if ( $question['user_answer'] != '' ) {
				$output .= nl2br( Football_Pool_Utils::xssafe( $question['user_answer'], null, false ) );
			} else {
			...
			if ( $this->always_show_predictions || ! $this->question_is_editable( $question['question_timestamp'] ) ) {
				...
				$output .= sprintf( '<p>%s: %s</p>',
									...
									( $question['user_answer'] != '' 
											? Football_Pool_Utils::xssafe( $question['user_answer'], null, false ) : '...' )
							);
				$show_admin_answer = Football_Pool_Utils::get_fp_option( 'user_page_show_correct_question_answer', false );
				if ( $show_admin_answer ) {
					if ( $question['question_answer'] !== '' ) {
						$question_answer = Football_Pool_Utils::xssafe( $question['question_answer'] );
					} else {
						$question_answer = ''; //__( 'unknown', 'football-pool' );
					}
					$output .= sprintf( '<p class="question-answer">%s: %s</p>',
										_x( 'correct answer', 'Label in front of correct answer on the User page', 'football-pool' ),
										$question_answer
								);
				}
				$output .= '</div>';
