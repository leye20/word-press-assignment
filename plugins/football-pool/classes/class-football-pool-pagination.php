	...
	public function show( $return = 'echo' ) {
		...
		if ( $this->show_total ) {
			$output .= sprintf( '<span class="displaying-num">%s</span>'
							, sprintf( _n( '1 item', '%s items', $this->total_items, 'football-pool' )
										, $this->total_items
								)
						);
		}
		...
		$output .= sprintf( '<span class="paging-input"><input class="current-page" title="%s" type="text" name="%s" value="%d" size="%d"> %s <span class="total-pages">%d</span></span>'
