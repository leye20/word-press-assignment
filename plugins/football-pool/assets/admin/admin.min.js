...
sub_iterations:b.sub_iterations,calculation_step:b.calculation_step,user:b.user,ranking:b.ranking,match:b.match,question:b.question,prev_total_score:b.prev_total_score,score_order:b.score_order,calc_start_time:b.calc_start_time},url:ajaxurl,global:!1,dataType:"json",method:"POST",success:function(c,e,f){null==c?t(FootballPoolAdmin.error_message+" Error: response was null."):(p="undefined"!==typeof c.completed&&1===c.completed,e=(new Date).getTime()-q,e=(e/b.iteration*b.total_iterations-e)/1E3,0>e&&
(e=0),1<b.iteration&&a("#time-left").html(r(e)),d=a("#fp-calc-progressbar"),0!=d.length&&(d.progressbar(),d.progressbar("option","max")!=c.total_iterations&&d.progressbar("option","max",c.total_iterations),d.progressbar("option","value",c.iteration)),a("#calculation-message").html(c.message),!1===c.error?p?(m(),a("#time-left").html(r(0)),a("#ajax-loader").hide()):k||l(c):(m(),t(c.error)))},error:function(a,c,b){t(b)}}))}function r(a){a=Math.floor(a);var b=Math.floor(a/3600),c=Math.floor((a-3600*b)/
60);a=a-3600*b-60*c;10>b&&(b="0"+b);10>c&&(c="0"+c);10>a&&(a="0"+a);return b+":"+c+":"+a}function m(){clearInterval(n);n=!1}function t(c){c=c||FootballPoolAjax.error_message;m();a("#ajax-loader").hide();a("#fp-calc-progress").show();a("#calculation-message").html('<span class="error">'+c+"</span>")}var h=[],q,n=!1,u=0,p=!1,k=!1;return{calculate:l,force_calculation:function(){u=1;l()},bulk_action_warning:function(c){var b=a("#"+c);return b&&0!=b.prop("selectedIndex")?(c=a("#"+c+" option").filter(":selected").attr("bulk-msg"),
""!=c&&void 0!=c?confirm(c):!0):!1},toggle_points:function(c){a("#"+c+"_points").toggle()},set_input_param:function(c,b,d){if(a.isArray(b)&&1<=b.length)a.each(b,function(b,f){""!==f&&(a.isArray(h[f])||(h[f]=[]),h[f][c]=a(f).attr(c),g=a.isArray(d)?d[b]:d,a(f).attr(c,g))});else if(""!==b){h[b]=[c,a(b).attr(c)];var g=a.isArray(d)?d[0]:d;a(b).attr(c,g)}},restore_input_param:function(c,b){var d="";a.isArray(b)&&1<=b.length?a.each(b,function(b,e){d=h[e][c];a(e).attr(c,d)}):""!==b&&(d=h[b][c],a(b).attr(c,
d))},disable_inputs:function(c,b){var d=b||"",g=!1;""!==d&&(g=a("#"+d).is(":checked"));a.isArray(c)&&1<=c.length?a.each(c,function(b,c){""!==c&&(""!==d?g?a(c).attr("disabled","disabled"):a(c).removeAttr("disabled"):a(c).attr("disabled","disabled"))}):""!==c&&(""!==d?g?a(c).attr("disabled","disabled"):a(c).removeAttr("disabled"):a(c).attr("disabled","disabled"))},toggle_linked_options:function(c,b){a.isArray(c)&&1<=c.length?a.each(c,function(b,c){""!==c&&a(c).toggle(!0)}):""!==c&&a(c).toggle(!0);a.isArray(b)&&
1<=b.length?a.each(b,function(c,b){""!==b&&a(b).toggle(!1)}):""!==b&&a(b).toggle(!1)}}}(jQuery);jQuery(document).ready(function(){jQuery("input[title].with-hint").inputHints()});jQuery.fn.inputHints=function(){jQuery(this).each(function(a){""===jQuery(this).val()&&jQuery(this).val(jQuery(this).attr("title")).addClass("hint")});return jQuery(this).focus(function(){jQuery(this).val()==jQuery(this).attr("title")&&jQuery(this).val("").removeClass("hint")}).blur(function(){""==jQuery(this).val()&&jQuery(this).val(jQuery(this).attr("title")).addClass("hint")})};
